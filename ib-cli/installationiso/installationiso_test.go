...
func TestInstallationIso(t *testing.T) {
	...
	testcases := []struct {
		...
		{
			name:               "Happy flow - skip shutdown set",
			workDirExists:      true,
			authFileExists:     true,
			pullSecretExists:   true,
			sshPublicKeyExists: true,
	...
	for _, tc := range testcases {
		...
		t.Run(tc.name, func(t *testing.T) {
			...
			err := installationIso.Create(seedImage, seedVersion, authFilePath, psFilePath, sshPublicKeyPath, lcaImage,
				rhcosLiveIsoUrl, installationDisk, extraPartitionStart, tc.precacheBestEffort, tc.precacheDisabled, tc.skipShutdown)
			if tc.expectedError == "" {
				assert.Equal(t, err, nil)
				...
				if tc.skipShutdown {
					assert.Equal(t, strings.Contains(string(data), "SKIP_SHUTDOWN"), true)
				} else {
					assert.Equal(t, strings.Contains(string(data), "SKIP_SHUTDOWN"), false)
				}
