...
const (
	...
	ibiConfigFileName         = "ibiConfiguration"
	authFilePath              = "/var/tmp/backup-secret.json"
	psFilePath                = "/var/tmp/pull-secret.json"
	ibiConfigFilePath         = "/var/tmp/ibi-configuration.json"
)
...
func (r *InstallationIso) renderButaneConfig(ibiConfig *ibiconfig.IBIPrepareConfig) error {
	...
	ibiConfig.AuthFile = authFilePath
	ibiConfig.PullSecretFile = psFilePath
	ibiConfiguration := path.Join(butaneFiles, ibiConfigFileName)
	if err := utils.MarshalToFile(ibiConfig, path.Join(r.workDir, ibiConfiguration)); err != nil {
		return fmt.Errorf("failed to marshal ibi config to file: %w", err)
	}

	templateData := IgnitionData{
		AuthFilePath:         authFilePath,
		PullSecretPath:       psFilePath,
		IBIConfigurationPath: ibiConfigFilePath,
		BackupSecret:         backupSecretInButane,
		PullSecret:           pullSecretInButane,
		SshPublicKey:         string(sshPublicKey),
		InstallSeedScript:    seedInstallScriptInButane,
		IBIConfiguration:     ibiConfiguration,
